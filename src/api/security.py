...
async def verify_api_key(api_key: str = Security(api_key_header)):
    ...
    if api_key not in VALID_API_KEYS:
        logger.warning(f"Từ chối truy cập: API Key không hợp lệ '{api_key}'")
        raise HTTPException(
...
def rate_limiter(request: Request):
    ...
    try:
        requests = redis_cache.client.incr(key)
        
        if requests == 1:
            redis_cache.client.expire(key, 60)
            
        if requests > settings.RATE_LIMIT_PER_MINUTE:
            logger.warning(f"Rate limit exceeded cho {identifier} ({requests} reqs/min)")
            raise HTTPException(
