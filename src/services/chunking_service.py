...
class ChunkingBatch:
    ...
    def get_table_preview(self, table_str: str, max_rows: int = 4) -> str:
        """
        Lấy thông tin bảng với vài đòng đầu để lưu vào vector store 
        """
        ...
        return "\n".join(preview_lines) + "\n| ... | ... (Bảng còn tiếp) |"
    def process_and_split(self, input_doc: Document) -> List[Document]:
        """
        Xử lý và chia nhỏ tài liệu thành các chunk.
        Args:
            input_doc (Document): Tài liệu cần xử lý.
        Returns:
            List[Document]: Danh sách các chunk đã được xử lý với các bảng biểu và children_text chunking
        """
        ...
        for parent in parent_docs:
            ...
            for block_id, table_content in tables_dict.items():
                if block_id in full_parent_content:
                    ...
                    table_meta = merged_metadata.copy()
                    table_meta["chunk_type"] = "table"
                    
                    children_chunk.append(
            ...
            if children_text:
                ...
                for child_text in child_splits:
                    contextualized_text = f"[Bối cảnh: {headers_context}]\n{child_text}" if headers_context else child_text
                    
                    text_meta = merged_metadata.copy()
